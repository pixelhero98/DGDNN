    ...
    for jj in ['gx']:
        ...
        for kk in ['1.0']:
            ...
            for vv in range(10):
                ...
                D1 = []
                D1, new_label = node_feature_label_generation(D1, 2016, 2016)
                rat = 0.5
                for index_iii, iii in enumerate(D1):
                    mark = D1.shape[1] / 5
                    D1[index_iii][- int(rat * mark):] = 0
                    D1[index_iii][- 2 * int(rat * mark): - int(rat * mark) - 1] = 0
                    D1[index_iii][- 3 * int(rat * mark): - 2 * int(rat * mark) - 1] = 0
                    D1[index_iii][- 4 * int(rat * mark): - 3 * int(rat * mark) - 1] = 0
                    D1[index_iii][- 5 * int(rat * mark): - 4 * int(rat * mark) - 1] = 0
                ...
                optimizer = torch.optim.Adam([dict(params=model.edgeconv1.parameters(), weight_decay=5e-5),
                                              ...
                                              dict(params=model.edgeconv3.parameters(), weight_decay=5e-5)], lr=2e-2)
